...
def tokenize(text, re_space, re_freeze, re_punct):
    spacesplitted = re_space.split(text)
    spacesplitted = [
    ...
    for substring in spacesplitted:
        if substring.isspace is True:
            ...
            puncts = re_punct.finditer(substring.text)
            s = []
            for i in puncts:
                s.extend([i.start(), i.end()])
            s = set(s)
            frozen = re_freeze.finditer(substring.text)
            for i in frozen:
...
text = texts.url
doc = tokenize(
    text, re_space=re_space, re_freeze=re_freeze, re_punct=re_punct
)
